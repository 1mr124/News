...
(function (root, factory) {
  ...
  // ---------------------------------------------------------------
  // User-channel backup (export/import). Export contains ONLY the
  // persistent channel definitions — never live state, cached video
  // metadata, or temporary discovery state. The version field lets the
  // format evolve later (import rejects unknown versions rather than
  // guessing). Both helpers are pure and never throw.
  // ---------------------------------------------------------------
  var USER_CHANNELS_EXPORT_VERSION = 1;
  var USER_CHANNELS_EXPORT_APP = "newstv-user-channels";

  // Strip a sanitized user row down to its persistent definition fields.
  ...
  function buildUserChannelsExport(channels, nowIso) {
    var clean = sanitizeUserChannels(channels).map(toExportRow);
    var ts = typeof nowIso === "string" && nowIso ? nowIso : new Date().toISOString();
    return {
      app: USER_CHANNELS_EXPORT_APP,
      version: USER_CHANNELS_EXPORT_VERSION,
      exportedAt: ts,
      count: clean.length,
      channels: clean,
    };
  ...
  // Validate an already-parsed JSON value as an import payload. Accepts the
  // current { app, version: 1, channels: [...] } shape (app/exportedAt/count
  // are informational only). Returns { ok: true, channels } with sanitized
  // rows, or { ok: false, error } — never throws, never touches storage.
  // Duplicate handling vs existing channels is the caller's job (via
  // findChannelDuplicate, canonical UC id first) so built-ins are never
  // overwritten here.
  function parseUserChannelsImport(data) {
    var list = null;
    if (Array.isArray(data)) {
      list = data; // tolerate a bare array (e.g. hand-edited backup)
    } else if (data && typeof data === "object") {
      if (!Array.isArray(data.channels)) {
        return { ok: false, error: "invalid" };
      }
      if (data.version !== USER_CHANNELS_EXPORT_VERSION) {
    ...
    var clean = sanitizeUserChannels(list);
    if (!clean.length && list.length) {
      return { ok: false, error: "invalid" };
    }
    return { ok: true, channels: clean.map(toExportRow) };
  }
  ...
  return {
    ...
    USER_CHANNELS_EXPORT_VERSION: USER_CHANNELS_EXPORT_VERSION,
    USER_CHANNELS_EXPORT_APP: USER_CHANNELS_EXPORT_APP,
    buildUserChannelsExport: buildUserChannelsExport,
    parseUserChannelsImport: parseUserChannelsImport,
