...
(function (root, factory) {
  ...
  function init(mountEl) {
    ...
    function renderView() {
      var url = targetUrl();
      var code = current ? current.code : URL.DEFAULT_CODE;
      // Show the language Google News will actually serve, not the one picked
      // — for countries with no Arabic edition build() substitutes another hl.
      var effLang = typeof URL.resolveHl === "function" ? URL.resolveHl(code, lang) : lang;
      var effLangLabel = typeof URL.hlLabel === "function" ? URL.hlLabel(effLang) : langLabel(effLang);
      var sub = effLangLabel + " · " + (query.trim() ? "Search: “" + query.trim() + "”" : topicLabel(topic));
      var note = typeof URL.editionNote === "function" ? URL.editionNote(code, lang) : "";
      frame.replaceChildren(el("div", { class: "empty gn-panel" }, [
        ...
        note ? el("div", { class: "gn-edition-note", text: "ⓘ " + note }) : null,
